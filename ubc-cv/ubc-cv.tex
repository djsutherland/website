...
%%
\vspace{-30pt}
\section{\dunderline{1pt}{POST-SECONDARY EDUCATION}}
% \subsection{Degrees}
% \noindent
\noindent\vspace{-2em}
\begin{table}[h!]
...
%% 6. EMPLOYMENT RECORD
%%
\section{\dunderline{1pt}{EMPLOYMENT RECORD}}
...
\nasection{%
  ...
  % 7. LEAVES OF ABSENCE
  %
  \section{\dunderline{1pt}{LEAVES OF ABSENCE}}
  \noindent\vspace{-2em}
  \begin{table}[h!]
    \bf
    % \begin{tabular}{||p{0.4\textwidth}||>{\centering\let\newline                                  \\\arraybackslash\hspace{0pt}}p{0.1\textwidth}||>{\centering\let\newline                                  \\\arraybackslash\hspace{0pt}}p{0.2\textwidth}||>{\centering\let\newline                                  \\\arraybackslash\hspace{0pt}}p{0.2\textwidth}||}
...
%% 8. TEACHING
%%
\section{\dunderline{1pt}{TEACHING}} \label{sec:teaching}
\subsection{Areas of special interest and accomplishments}
% UBC rules: 150 words max

  \paragraph{CPSC 532S: Modern Statistical Learning Theory.}
  \new{This new graduate course focuses on
...
\begin{coursetable}
  \newtable 21-22 W2  & CPSC 532S & 3 & 23 & 3/week  & 0 & 0 & 2h/week office hours \\
  \newtable 21-22 W1  & CPSC 340  & 3 & 89\textit{\small +139} & 3/week  & 0 & 0 & 1h/week office hours \\
\end{coursetable}
\tremark{CPSC 340 in 2021-22 W1 was run jointly with another section taught by Mike Gelbart with an enrolment of 139, for a total of 228 students across the two sections.}
...
% \paragraph{Graduate Supervisory Committees}
% \begin{supcommtable}
%   \newtable Minglong Li         & PhD UBC       & 2021/08 & 2022/02 & RPE Committee Member         \\
% \end{supcommtable}
...
%%
\section{\dunderline{1pt}{SCHOLARLY AND PROFESSIONAL ACTIVITIES}}
\subsection{Areas of special interest and accomplishments} % UBC rules: 150 words max

My research focuses on representation learning, broadly construed.
Major recent areas of progress:
...
Recent breakthroughs include justifying an existing method (previously motivated by a very loose connection to mutual information) as closely approximating a deep kernel method,
and further progress in deep kernel two-sample testing.
}

\paragraph{Uniform convergence of high-dimensional linear models.}
\continuing{
Surprising doubts have recently been cast cast on traditional learning theory's ability to explain behaviour in high-dimensional regimes.
We have recently shown that a variation known as \emph{optimistic rates} are, however, capable of almost fully explaining the behaviour of high-dimensional linear models such as linear regression for Gaussian data (a surprisingly difficult task), a direction other theorists are actively building on.}


\subsection{Research or equivalent grants (indicate under COMP whether
  grants were obtained competitively (C) or non-competitively (NC))}
...
\begin{granttable}
  % \newtable
  % NVIDIA & GPU Grant & C-reject & -- & 2022 & D.~J.~Sutherland \\
  ...
  \midrule
  % \newtable
  % Microsoft Research & Faculty Fellowship & C-reject & -- & 2021 & D.~J.~Sutherland \\
  % % TODO: does this belong here?
  % \midrule
  \newtable
...
\nasubsection{%
  \subsection{Research or equivalent contracts (indicate under COMP whether
    grants were obtained competitively (C) or non-competitively (NC))}
  \begin{granttable}
    N/A &  &  &  &   & \\
  \end{granttable}
...
\begin{itemize}[leftmargin=5em]
  ...
  \item[2022] \new{D\&I event organizer (Queer in AI), AAAI Conference on Artificial Intelligence (AAAI)}
  \item[2020-22] \changed{Area Chair, Conference on Neural Information Processing Systems (NeurIPS)}
  \item[2020-22] \changed{Area Chair, Conference on Artificial Intelligence and Statistics (AISTATS)}
  \item[2021-22] \changed{Senior Program Committee, AAAI Conference on Artificial Intelligence (AAAI)}
\end{itemize}
...
%%
% \section{\dunderline{1pt}{SERVICE TO THE UNIVERSITY AND THE COMMUNITY}}
...
\section{\dunderline{1pt}{SERVICE TO THE UNIVERSITY}}
...
a significant problem for e.g.\ trans people who (like me) are not Canadian permanent residents and thus cannot change their legal name in BC.
I have a list of over a dozen,
...
\begin{itemize}[leftmargin=5em]
  \item[2022] \new{Faculty recruiting committee: CRC Tier 2 in Quantum Computing, Computer Science Department}
...
%%
\section{\dunderline{1pt}{SERVICE TO THE COMMUNITY}}
...
and \new{have been a core organizer since January 2022.}
My work through the organization has included
trans-inclusive advocacy to conferences
(including \new{enormous improvements to handling of preferred names at NeurIPS/ICML/ICLR/AISTATS/MLSys} and \continuing{improved post-publication name change policies at several venues});
\continuing{co-organizing social, mentoring, and workshop events at conferences};
\new{reviewing administrator applications as part of a hiring committee};
\continuing{initiating an informal program of feedback and mentoring for faculty candidates};
and more.

\nasubsection{
\subsection{Memberships on scholarly societies, including offices held
  and dates}
\begin{itemize}[leftmargin=5em]
...
\nasubsection{%
  \subsection{Memberships on other societies, including offices held
    and dates}
...
\nasubsection{%
  \subsection{Memberships on scholarly committees, including offices held
    and  dates}
...
\nasubsection{%
  \subsection{Memberships on other committees, including offices held
    and dates}
...
\begin{itemize}[leftmargin=5em]
  \item[2022] Conference reviewing: \new{UAI}, \continuing{ICML}.
  \item[2021] Journal reviewing: \continuing{JMLR}, \new{Bernoulli}.
...
%%
\section{\dunderline{1pt}{AWARDS AND DISTINCTIONS}}
...
\begin{itemize}[leftmargin=5em]
  \item[2021] \new{[C18]: NeurIPS oral (55 of 9,122 submissions, 0.6\%)}
...
%% Maximum One Page
\nasection{
  \section{\dunderline{1pt}{OTHER RELEVANT INFORMATION}}
...
(Informal co-supervisees, discussed in section 8 \ref{sec:other-sup}, not included.)
...
\section{\dunderline{1pt}{REFEREED PUBLICATIONS}}
...
Most papers of these papers have (or will have) a corresponding conference version above.}
...
\section{\dunderline{1pt}{NON-REFEREED PUBLICATIONS}}
...
\nasection{%
\section{\dunderline{1pt}{PATENTS}}
...
\nasection{%
  \section{\dunderline{1pt}{SPECIAL COPYRIGHTS}}

  Not applicable.
}%

\nasection{%
  \section{\dunderline{1pt}{ARTISTIC WORKS, PERFORMANCES, DESIGNS}}

  Not applicable.
}%

\nasection{%
  \section{\dunderline{1pt}{OTHER WORKS}}

  Not applicable.
}%

\nasubsection{
\section{\dunderline{1pt}{WORK SUBMITTED}} % (including publisher and date of submission)}
...
\nasection{%
  \section{\dunderline{1pt}{WORK IN PROGRESS} (including degree of completion)}
